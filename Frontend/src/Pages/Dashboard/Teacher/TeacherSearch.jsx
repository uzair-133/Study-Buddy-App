...
const TeacherSearch = () => {
  ...
  const getResult = async (searchTerm, cat) => {
    ...
    try {
      ...
      const res = await api.get(
        `/api/search/?category=${cat}&keyword=${encodeURIComponent(searchTerm.trim())}`,
        {
  ...
  const totalResults =
    subjectsList.length + chaptersList.length + materialsList.length;
    ...
    switch (catType) {
      case "lecture":
        return {
          label: "Lecture Notes",
          icon: FileText,
          style: "bg-violet/10 text-violet border-violet/20",
        };
      case "handwritten":
        return {
          label: "Handwritten",
          icon: PenTool,
          style: "bg-coral/10 text-coral border-coral/20",
        };
      case "slide":
        return {
          label: "Slides",
          icon: Presentation,
          style: "bg-mint/10 text-mint border-mint/20",
        };
      ...
      default:
        return {
          label: "File",
          icon: FileText,
          style: "bg-gray-100 text-ink-soft border-gray-200",
        };
    }
  };
    ...
    <section className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-ink-soft font-sans text-xs sm:text-sm mt-1">
            Search notes, chapters, slides, and subjects across your entire
            workspace.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 sm:p-6 rounded-2xl border border-line shadow-xs space-y-4">
        <div className="relative flex items-center">
          <Search
            className="absolute left-4 text-violet pointer-events-none"
            size={20}
          />
          <input
            type="text"
        ...
        <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-line/60">
          <span className="text-xs font-semibold text-ink-soft uppercase tracking-wider mr-1">
            Filter:
          </span>

          <button
            onClick={() => setCategory("all")}
            className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs sm:text-sm font-sans font-semibold transition-all cursor-pointer ${category === "all"
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs sm:text-sm font-sans font-semibold transition-all cursor-pointer ${category === "subject"
                ? "bg-violet text-white shadow-xs"
                : "bg-paper text-ink-soft hover:bg-gray-200 border border-gray-200"
              }`}
          >
            ...
            {hasSearched && (
              <span
                className={`ml-1 text-xs px-1.5 py-0.5 rounded-full ${category === "subject" ? "bg-white/20 text-white" : "bg-gray-200 text-ink-soft"}`}
              >
                {subjectsList.length}
              </span>
            )}
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs sm:text-sm font-sans font-semibold transition-all cursor-pointer ${category === "chapter"
                ? "bg-violet text-white shadow-xs"
                : "bg-paper text-ink-soft hover:bg-gray-200 border border-gray-200"
              }`}
          >
            ...
            {hasSearched && (
              <span
                className={`ml-1 text-xs px-1.5 py-0.5 rounded-full ${category === "chapter" ? "bg-white/20 text-white" : "bg-gray-200 text-ink-soft"}`}
              >
                {chaptersList.length}
              </span>
            )}
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs sm:text-sm font-sans font-semibold transition-all cursor-pointer ${category === "files"
                ? "bg-violet text-white shadow-xs"
                : "bg-paper text-ink-soft hover:bg-gray-200 border border-gray-200"
              }`}
          >
            ...
            {hasSearched && (
              <span
                className={`ml-1 text-xs px-1.5 py-0.5 rounded-full ${category === "files" ? "bg-white/20 text-white" : "bg-gray-200 text-ink-soft"}`}
              >
      ...
      {error && (
        <div className="bg-coral/10 border border-coral/20 text-coral p-4 rounded-xl text-sm flex items-center justify-between">
          <p>{error}</p>
          <button
            onClick={() => getResult(keyword, category)}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[1, 2, 3, 4].map((n) => (
              <div
                key={n}
                className="bg-white border border-line rounded-2xl p-5 animate-pulse space-y-3"
              >
        ...
        <div className="bg-paper/60 border border-line rounded-2xl p-6 text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-ink-soft text-sm max-w-md mx-auto mt-1">
              Start typing above to quickly locate subjects, chapters, or files
              uploaded in your study buddy portal.
            </p>
          </div>
          ...
          <div className="pt-2">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2">
              {["Notes", "Lecture", "Assignment", "Exam", "Chapter 1"].map(
                (term) => (
                  <button
                    key={term}
        ...
        <div className="space-y-8">
          ...
          <div className="flex items-center justify-between text-sm text-ink-soft border-b border-line pb-2">
            <span>
              ...
              {totalResults === 1 ? "" : "s"} for "<strong>{keyword}</strong>"
            </span>
          </div>

          {/* Subjects Section */}
          {(category === "all" || category === "subject") &&
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {chaptersList.map((chap) => {
                    const parentSubjectId =
                      chap.subjectId?._id || chap.subjectId;
                    const parentSubjectTitle =
                      chap.subjectId?.title || "Subject";
                      ...
                      >
                        <div>
                          <div className="flex items-center justify-between gap-2 mb-2">
                            ...
                            <span
                              className="text-xs font-semibold text-ink-soft truncate max-w-37.5"
                              title={parentSubjectTitle}
                            >
                              {parentSubjectTitle}
                            </span>
                          </div>
                        ...
                        {parentSubjectId ? (
                          <Link
                            to={`/teacher/subjects/${parentSubjectId}/chapters/${chap._id}`}
                            className="mt-4 flex items-center justify-between text-xs font-semibold text-violet hover:underline pt-3 border-t border-line/60"
                          >
                      ...
                      >
                        <div className="space-y-2">
                          <div className="flex items-center justify-between gap-2">
                            <span
                              className={`inline-flex items-center gap-1 text-xs font-bold px-2.5 py-0.5 rounded-full border ${badge.style}`}
                            >
                              <IconComp size={12} />
                              {badge.label}
                            </span>
                          ...
                          {(subjectTitle || chapterTitle) && (
                            <div className="flex items-center gap-1 text-xs text-ink-soft">
                              <FolderOpen
                                size={12}
                                className="text-violet/70 shrink-0"
                              />
                        ...
                        <div className="flex items-center justify-between gap-2 pt-3 border-t border-line/60">
                          {mat.fileUrl ? (
                            <button
                              type="button"
                              onClick={() => openFileViewer(mat.fileUrl, mat.fileName)}
                              className="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl bg-violet/10 hover:bg-violet hover:text-white text-violet text-xs font-semibold transition-all cursor-pointer"
                            >
                          ...
                          {mat.subjectId?._id && mat.chapterId?._id && (
                            <Link
                              to={`/teacher/subjects/${mat.subjectId._id}/chapters/${mat.chapterId._id}`}
