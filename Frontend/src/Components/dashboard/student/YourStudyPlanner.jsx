
import { Link, useLocation } from 'react-router-dom'
import api from '../../../api/axios'
import { useEffect, useState } from 'react'
const YourStudyPlanner = () => {
  const [tasks, setTask] = useState([])
  const [error, setError] = useState('')
  const [loading, setLoading] = useState(true)
  const location = useLocation()
  const basePath = location.pathname.startsWith('/teacher') ? '/teacher' : '/student'

  const getTask = async () => {
    try {
      const res = await api.get('/api/task/get')
      setTask(res.data.task || [])
      setError('')
    }
    catch (err) {
      setError(err.response?.data?.message || 'Failed to fetch tasks')

    } finally {
      setLoading(false)
    }
  }
  useEffect(() => {
    getTask();
  }, [])


  return (
    <>
      <section className='bg-white rounded-2xl p-4 border  border-gray-300 '>
        <div className='flex justify-between'>
          <h1 className='font-semibold font-display text-sm'>Study Planner </h1>
          <Link to={`${basePath}/study-planner`} className='text-violet text-sm font-display hover:underline'>View all</Link>
        </div>
        <div>
          {
            tasks.map((e, index) => {
              return (
                <div key={e._id || index} className='flex space-x-2 space-y-2 mt-3 border-b border-gray-200'>
                  <div className='rounded-md bg-gray-100 leading-4 px-2 py-1  font-semibold font-display text-[13px] '>
                    <span>                                   
                     {new Date(e.date).toLocaleDateString(undefined, { day: 'numeric', month: 'short' })}
                    </span>
                  </div>
                  <div>
                    <p className='text-sm font-sans font-semibold'>{e.title}</p>
                    <p className='text-ink-soft text-sm font-sans'>{e.note}</p>

                  </div>
                </div>
              )
            })
          }
        </div>
        <div className='mt-5 flex justify-center'>
          <Link to={`${basePath}/study-planner`} className=' px-8 py-2 text-sm sm:px-14  rounded-md outline-dashed  outline-2 outline-gray-300 text-center   font-semibold font-sans text-violet'>
            +Add Study Task
          </Link>
        </div>

      </section>
    </>
  )
}

export default YourStudyPlanner


