...
const UploadMaterialModal = ({ chapterId, subjectId, initialCategory = 'lecture', onClose, onMaterialUploaded }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 backdrop-blur-xs p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md border border-line p-6 relative animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex justify-between items-center pb-4 border-b border-line">
          <h2 className="font-display font-bold text-lg text-ink">Upload Study Material</h2>
          <button
            onClick={onClose}
            disabled={loading}
              ...
              <div className="flex flex-col items-center justify-center gap-2">
                ...
                {file ? (
                  <div className="flex items-center gap-2 text-violet font-semibold text-xs">
                    ...
                    <span className="text-ink-soft font-normal">({(file.size / (1024 * 1024)).toFixed(2)} MB)</span>
                  </div>
                ) : (
                  <>
                    <p className="text-xs font-semibold text-ink">
                      Click to upload <span className="text-ink-soft font-normal">or drag & drop</span>
                    </p>
                    <p className="text-[11px] text-ink-soft">PDF, DOCX, PPT, PNG, JPG (Max 20MB)</p>
                  </>
                )}
              </div>
            </div>
